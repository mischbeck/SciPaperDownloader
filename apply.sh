...
original_author="mischbeck"
original_name="scipaperdownloader"
original_urlname="SciPaperDownloader"
original_description="Awesome scipaperdownloader created by mischbeck" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/scipaperdownloader "${name}"
...
then
    ...
    echo "then run 'scipaperdownloader' or 'python -m scipaperdownloader'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'scipaperdownloader' or 'python -m scipaperdownloader' or 'poetry run scipaperdownloader'"
fi
