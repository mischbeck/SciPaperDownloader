...
- 🎯 Entry points to execute your program using `python -m <scipaperdownloader>` or `$ scipaperdownloader` with basic CLI argument parsing.
...
---
# scipaperdownloader Flask Application

Awesome scipaperdownloader created by mischbeck
...
```bash
git clone https://github.com/mischbeck/SciPaperDownloader scipaperdownloader
cd scipaperdownloader
make install
```
...
```bash
pip install scipaperdownloader
```
...
```bash
scipaperdownloader create-db   # run once
scipaperdownloader populate-db  # run once (optional)
scipaperdownloader add-user -u admin -p 1234  # ads a user
scipaperdownloader run
```
